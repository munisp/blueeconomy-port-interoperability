...
# Payment intents, workflow starts and the TigerBeetle refund rail need the
# real Mojaloop/Temporal/TigerBeetle edges; they are configured with
# fail-closed values here and are not invoked by this script (the booking
# paths verified below need only PostgreSQL — the cancelled booking below is
# unpaid, so no refund leg is ever attempted).
...
export DATABASE_URL \
  ...
  TIGERBEETLE_ADDRESSES='127.0.0.1:1' \
